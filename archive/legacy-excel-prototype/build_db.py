...
headers = ['표준품목ID', '품목분류', '품명', '규격', '단위',
           ...
           '데이터건수', '주요메이커', '최근견적일', '출처공급사', '데이터입력일자']
widths   = [12, 10, 35, 30, 8, 15, 15, 15, 10, 15, 12, 25, 14]
...
for ri, rec in enumerate(db_records, 4):
    ...
    vals = [rec['표준품목ID'], rec['품목분류'], rec['품명'], rec['규격'], rec['단위'],
            ...
            rec.get('데이터입력일자', '')]
    alns = [center, center, left, left, center, right, right, right, center, left, center, left, center]
...
ws.auto_filter.ref = f"A3:M{3 + len(db_records)}"
ws.freeze_panes = 'A4'

# 원본 데이터 시트
ws2 = wb.create_sheet("원본견적데이터")
h2 = ['ID', '공급사', '견적번호', '견적일', '공사명', '단위장비명', '품목분류',
      '품명', '규격', '단위', '수량', '단가(원)', '금액(원)', '메이커', '데이터입력일자']
w2 = [6, 14, 20, 12, 25, 25, 10, 30, 25, 8, 8, 15, 15, 15, 14]
...
for ri, it in enumerate(all_items, 2):
    ...
    for ci, v in enumerate(vals, 1):
        ...
        cell.font = Font(name='맑은 고딕', size=9)
        cell.border = border
        cell.alignment = center if ci in [1, 2, 3, 4, 7, 10, 11, 15] else left
    for ci in [12, 13]:
        ws2.cell(row=ri, column=ci).number_format = '#,##0'
    ws2.row_dimensions[ri].height = 18

ws2.auto_filter.ref = f"A1:O{1 + len(all_items)}"
ws2.freeze_panes = 'A2'

import os
out = _here / "표준단가DB.xlsx"
tmp = _here / "표준단가DB_new.xlsx"
wb.save(str(tmp))
