...
추가로, 산정 근거 부족(날짜 미확인·지수 없음 등)으로 목표가가 계산되지 않는 품목이 섞여 있으면
합계 행의 "구매 금액" 칸(`totalQuoteAmount`, 전체 품목 합)과 "목표 금액" 칸(`totalTargetAmount`,
목표가 산정된 품목만 합)이 서로 다른 품목 집합을 기준으로 계산된다. "목표가 대비" 합계 셀 자체는
내부적으로 산정 가능한 품목의 구매금액만 걸러서(`targetCoveredQuoteAmount`) 정확히 계산하지만,
바로 옆 "구매 금액" 합계 칸은 전체 합이라 두 칸을 육안으로 비교(암산으로 구매금액 − 목표금액)하면
틀린 값이 나온다. 비교 기준이 되는 품목 집합이 다르다는 걸 화면에서 명시해야 한다.
...
- 합계(`tfoot`) 행의 신규 컬럼 칸은 `—` 처리 — 서로 다른 품목의 개당 차액을 단순 합산하는 것은
  의미가 없음.
- **비교 기준 명시**: 기존 "합계" 행 바로 위에 서브토탈 행을 하나 추가한다.
  - 라벨(첫 3칸 `colSpan`): "산정 대상 구매금액(목표가 있는 품목만)"
  - "구매 금액" 컬럼 위치에 `targetCoveredQuoteAmount` 값을 표시 — 바로 아래 "합계" 행의
    "구매 금액"(`totalQuoteAmount`, 전체 품목 합)과 같은 컬럼에 겹쳐 보여서 두 숫자가 서로 다른
    기준(대상 품목만 vs 전체 품목)임을 한눈에 비교할 수 있게 한다.
  - 나머지 숫자 컬럼(구매 목표 단가·목표 금액·목표가 대비·목표가 대비(개당))은 `—` — 이미 바로
    아래 "합계" 행에 목표가 산정 대상 기준으로 나와 있으므로 중복 표기하지 않는다.
  - "합계" 행의 기존 "구매 금액" 칸(`totalQuoteAmount`, 전체 품목 합)은 값·의미 변경 없이 유지 —
    실제 견적서 총액을 보여주는 용도로 계속 필요함.
