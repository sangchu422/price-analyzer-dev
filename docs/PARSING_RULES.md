...
- `견적서/3차 학습/`의 AONE·바츠 원본은 현재 코퍼스의 일부다.
- XLS/XLSX는 공통 헤더 탐지와 고정열 보조 규칙으로 읽는다.
- PDF는 먼저 pypdf 안전성·자원 한도 검사를 통과해야 한다. 현대위아
  통일양식의 `단위공사명`, `단위장비명`, `전기부문` 표식이 있으면
  pdfplumber 표 추출을 보조로 사용한다.
- 보조 파서가 만든 행에는 `PDF_WIA_TABLE`과 단위 구역명이 경고 증빙으로
  남으며, PDF 페이지까지 역추적할 수 있다.
- 손상·DRM·비지원 양식은 조용히 누락하지 않고 적재 보고서의 파일별 실패로
  남긴다. 실패 파일은 자동 표준가격 근거로 사용하지 않는다.

## 실행

```bat
cd backend
..\.venv\Scripts\python.exe -m app.cli preflight --quote-root ..\견적서
..\.venv\Scripts\python.exe -m app.cli ingest --quote-root ..\견적서 --database-file .local\standard-item-migration-v2.sqlite3
..\.venv\Scripts\python.exe -m app.cli standard-db-build --database-file .local\standard-item-migration-v2.sqlite3
```
