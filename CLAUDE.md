...
작업을 시작하면 먼저 `docs/HANDOFF_2026-08-11.md`의
...
4. 표준 DB에 가격 근거가 있으면 신규 견적 단가를 비교한다. 내부 매칭이
   없으면 DeviceMart·Mouser 시장가 캐시를 우선 조회하고, 미보유·만료
...
DeviceMart·Mouser 시장가 캐시·증빙·웹 조회도 구현 완료했으며 Mouser는
루트 `.env`의 `MOUSER_API_KEY` 설정이 필요하다. 서버 설치는 후속 단계다.
