...
작업을 시작하면 먼저 `docs/HANDOFF_2026-08-18.md`의
`사내 환경 첫 작업 체크리스트`를 읽고 현재 브랜치·DB·실행 상태를
...
2. 최신 `INCLUDED` 행만 품명·사양·단위 기준으로 그룹화해 내부 표준 DB를
   자동 구축한다.
3. 웹 `/analysis`에서 신규 견적을 업로드한다. 신규 문서는 기본적으로
   `INCOMING_BID`에 격리한다. 담당자와 반영 사유를 입력한 명시적 승인에서만
   문서 역할·멤버십·가격 버전·감사 이력을 append-only로 추가한다.
4. 표준 DB에 가격 근거가 있으면 신규 견적 단가를 비교한다. 내부 매칭이
   없으면 DeviceMart 시장가 캐시를 우선 조회하고, 미보유·만료
   품목만 실시간 조회한다. 외부 근거도 없으면 검토 대기로 남긴다.
...
DeviceMart 시장가 캐시·증빙·웹 조회도 구현 완료했다. 서버 설치는 후속 단계다.
