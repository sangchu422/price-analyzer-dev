...
상세 절차와 예상 건수는 `docs/HANDOFF_2026-08-18.md`의
`다른 PC에서 pull 후 최초 설정`을 따른다.
...
견적은 기본적으로 `INCOMING_BID`에 격리되어 표준 DB의 멤버나 가격 근거가
되지 않는다. 분석 화면에서 담당자와 반영 사유를 입력해 명시적으로 승인한
경우에만 문서 역할·멤버십·가격 버전·감사 이력을 append-only로 추가한다.
승인 없이 자동 반영되는 경로는 없다.
